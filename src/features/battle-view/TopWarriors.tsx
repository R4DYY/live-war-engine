...
import type { WarriorContribution } from "@/live/warriorStore";

interface TopWarriorsProps {
  teamName: string;
  teamColor: string;
  side: "left" | "right";
  warriors: WarriorContribution[];
}
...
export function TopWarriors({ teamName, teamColor, side, warriors }: TopWarriorsProps) {
  const top3 = useMemo(() => {
    ...
    return sorted.length > 0
      ? sorted
      : Array.from({ length: 3 }, (_, i) => ({ userId: "", username: i === 0 ? "Awaiting champion" : "—", team: "A" as const, totalCoins: 0, giftCount: 0, lastGiftAt: 0, firstSeenAt: 0 }));
  }, [warriors]);
