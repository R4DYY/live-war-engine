...
import { GiftToastOverlay } from "./GiftToastOverlay";
...
export default function BattlePage() {
  ...
  const teamAWarriors = useWarriorStore((s) => s.teamA);
  const teamBWarriors = useWarriorStore((s) => s.teamB);
  const resetWarriors = useWarriorStore((s) => s.reset);
  const resetToasts = useGiftToastStore((s) => s.reset);
  ...
  return (
    <BattleStage>
      ...
      <div className="relative min-h-0 overflow-hidden" style={{ flex: "65 1 0" }}>
        ...
        {isVoting && (
          <NextGameVotingScreen
            onBattleStarting={(duel) => setNextDuel(duel.id)}
            currentDuelId={session?.config.duelId ?? null}
            recentDuelIds={loop.recentDuelIds}
          />
        )}
