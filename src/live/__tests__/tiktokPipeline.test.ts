...
import { normalizeUsername, validateUsername, resolveBridgeUrl } from "@/live/tiktokBridgeClient";
...
describe("TikTok live pipeline helpers", () => {
  ...
  it("normalizes TikTok usernames before sending commands", () => {
    expect(normalizeUsername("  @@underpressurefits  ")).toBe("underpressurefits");
    expect(validateUsername("  ")).toBeNull();
    expect(validateUsername("@creator")).toBe("creator");
  });

  it("uses the configured secure bridge URL", () => {
    expect(resolveBridgeUrl()).toBe("wss://live-war-engine-production.up.railway.app/live-bridge");
  });
});
