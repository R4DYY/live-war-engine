  ...
  connect(): void {
    ...
    this.socket.onopen = () => {
      ...
      for (const msg of queue) {
        this.socket?.send(JSON.stringify(this.createMessage(msg.type, msg.payload)));
      }
    ...
    this.socket.onerror = () => {
      this.updateBridgeState({ status: "ERROR", errorMessage: "TikTok bridge connection error" });
    };
  }
